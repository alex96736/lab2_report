...
gp <- ggplot(data = DT.import, aes(x = as.factor(DT.import$Year),
                              y = DT.import$Netweight.kg.mean,
                              color = DT.import$month)
...
boxplot(DT.import$Netweight.kg.mean ~ as.factor(DT.import$Year) * as.factor(DT.import$month),
        xlab = 'Масса поставки, кг',
        ylab = 'Плотность распределения',
        main = 'пакет base'
)
